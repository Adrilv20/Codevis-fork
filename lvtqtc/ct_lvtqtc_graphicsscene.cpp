...
void GraphicsScene::populateMenu(QMenu& menu, QMenu *debugMenu)
{
    ...
    if (d->pluginManager) {
        ...
        auto getEdgeByQualifiedName = [this](std::string const &fromQualifiedName, std::string const &toQualifiedName) -> std::optional<Edge> {
        ...
        auto registerContextMenu =
            [this, &menu, getAllEntitiesInCurrentView, getEntityByQualifiedName, getEdgeByQualifiedName](std::string const &title,
                                                                                                         ctxMenuAction_f const &userAction) {
                auto *action = menu.addAction(QString::fromStdString(title));
                connect(action, &QAction::triggered, this, [this, userAction, getAllEntitiesInCurrentView, getEntityByQualifiedName, getEdgeByQualifiedName]() {
                    ...
                    auto handler =
                        PluginContextMenuActionHandler{getPluginData, getAllEntitiesInCurrentView, getEntityByQualifiedName, getTree, getEdgeByQualifiedName};
                    userAction(&handler);
                });
            };

        auto& pm = d->pluginManager.value().get();
        pm.callHooksContextMenu(getAllEntitiesInCurrentView, getEntityByQualifiedName, getEdgeByQualifiedName, registerContextMenu);
    }
