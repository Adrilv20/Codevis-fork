...
TEST_CASE("Plugin manager")
{
    ...
    qDebug() << "pm.loadPlugins\n";

    REQUIRE(pm.getPluginById(CPP_TEST_PLUGIN_ID)->get().pluginId() == CPP_TEST_PLUGIN_ID);
    qDebug() << "OK[1]\n";
    REQUIRE(pm.getPluginById(PY_TEST_PLUGIN_ID)->get().pluginId() == PY_TEST_PLUGIN_ID);
    qDebug() << "OK[2]\n";
